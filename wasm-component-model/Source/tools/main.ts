	...
	try {
		if (options.stdin) {
			...
			if (stat.isFile() && path.extname(options.input) === ".json") {
				...
				stat.isDirectory() ||
				(stat.isFile() && path.extname(options.input) === ".wit")
			) {
				try {
					...
					try {
						data = cp.execFileSync(
							"wasm-tools",
							["component", "wit", "--json", options.input],
							{ shell: true, encoding: "utf8" },
						);
					} catch (error: any) {
...
export async function main(): Promise<number> {
	yargs
		...
		.exitProcess(false)
		.usage(
			`Tool to generate a TypeScript file and the corresponding meta data from a WIT JSON file.\nVersion: ${require("../../package.json").version}\nUsage: wit2ts [options] [wasi.json]`,
		)
		.example(
			`wit2ts --outDir . wasi.json`,
			`Creates a TypeScript file for the given Wit JSON file and writes the files into the provided output directory.`,
		)
		.example(
			`wit2ts --outDir . ./wit`,
			`Creates a TypeScript file for the files in the given wit directory and writes the files into the provided output directory. Requires a wasm-tools installation.`,
		)
		.version(false)
		...
		.option("filter", {
			description:
				"A regular expression to filter the packages to be included.",
			string: true,
		})
		.option("target", {
			description:
				"The target language. Currently only TypeScript is supported.",
			enum: ["ts"],
			default: "ts",
		})
		.option("nameStyle", {
		...
		.option("structure", {
			description:
				"By default wit2ts only generate the structure necessary to make names unique. This options force wit2ts to generate the package and or namespace names, even if they are not necessary.",
			enum: ["auto", "package", "namespace"],
			default: "auto",
		})
		.command(
			"$0 [input]",
			"Process the JSON file or WIT directory",
			(yargs) => {
