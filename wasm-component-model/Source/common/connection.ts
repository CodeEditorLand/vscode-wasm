...
class ConnectionMemory implements Memory {
	...
	constructor(
		...
		if (typeof sizeOrBuffer === "number") {
			...
			this.next = new Uint32Array(
				this.buffer,
				ConnectionMemory.Header.next.offset,
				1,
			);
	...
	public realloc(): MemoryRange {
		throw new ComponentModelTrap(
			"ConnectionMemory does not support realloc",
		);
	}
...
export abstract class Connection {
	...
	protected static serializeParams(
	...
	protected static deserializeParams(
	...
	protected static serializeResult(
		result: WasmType | undefined | void,
	...
	protected static deserializeResult(
	...
	protected static loadResult(
		buffer: SharedArrayBuffer,
	): WasmType | void | undefined {
	...
	protected static storeResult(
		buffer: SharedArrayBuffer,
		result: WasmType | void | undefined,
	): void {
		const view = new DataView(
			buffer,
			0,
		...
		if (result === undefined) {
			view.setUint32(
				ConnectionMemory.Header.resultType.offset,
				Connection.WasmTypeKind.undefined,
				true,
			);
			...
			if (result < 0) {
				view.setUint32(
					ConnectionMemory.Header.resultType.offset,
					Connection.WasmTypeKind.signed,
					true,
				);
				view.setBigInt64(
					ConnectionMemory.Header.result.offset,
				...
				view.setUint32(
					ConnectionMemory.Header.resultType.offset,
					Connection.WasmTypeKind.unsigned,
					true,
				);
				view.setBigUint64(
					ConnectionMemory.Header.result.offset,
...
export abstract class BaseWorkerConnection
	extends Connection
	implements WorkerConnection
{
	...
	private readonly handlers: Map<
		string,
		(memory: Memory, params: WasmType[]) => WasmType | void
	>;
	...
	public on(
		name: string,
		handler: (memory: Memory, params: WasmType[]) => WasmType | void,
	): void {
	...
	public callMain(name: string, params: WasmType[]): WasmType | void {
		...
		switch (result) {
			...
			case "not-equal":
				...
				if (value !== 1) {
					throw new ComponentModelTrap(
						`Unexpected value ${value} in sync object`,
					);
				}
		}
	...
	protected handleMessage(message: Connection.WorkerMessages): void {
		if (message.method === "initializeWorker") {
			...
			RAL()
				.WebAssembly.instantiate(message.module, imports)
				.then((instance) => {
					wasmContext.initialize(
						new Memory.Default(instance.exports),
					);
					$exports.worker.bind(
						this,
						this.world,
			...
			try {
				const memory: Memory = new ConnectionMemory(
				...
				this.postMessage({
					method: "reportResult",
					name: message.name,
					result: Connection.serializeResult(result),
				});
			} catch (error) {
				this.postMessage({
					method: "reportResult",
					name: message.name,
					error: `Calling WASM function ${message.name} failed.`,
				});
			}
		}
	}
...
export abstract class BaseMainConnection
	extends Connection
	implements MainConnection
{
		...
		if (message.method === "callMain") {
			...
			if (handler === undefined) {
				view.setUint32(
					...
					Connection.ErrorCodes.noHandler,
					true,
				);
				...
				if (result instanceof Promise) {
					result
						...
						.catch(() => {
							view.setUint32(
								ConnectionMemory.Header.errorCode.offset,
								Connection.ErrorCodes.promiseRejected,
								true,
							);
						})
						.finally(() => {
