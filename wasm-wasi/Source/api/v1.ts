 ...
 */
export type ProcessOptions = BaseProcessOptions &
	(MountPointOptions | RootFileSystemOptions);
...
export namespace Wasm {
	...
	async function doLoad(
		wasiCoreExt: Extension<APILoader | Wasm>,
	): Promise<Wasm> {
		try {
			...
			try {
				...
				throw new Error(
					`WASM WASI Core extension version ${api.versions.extension} is not compatible with this API version ${1}`,
				);
			} catch (error) {
				$api = null;
