...
namespace Converter {
	export function asDocumentFilter(
		value: Types.DocumentFilter,
	...
	export function asDocumentFilters(
		value: Types.DocumentFilter[],
	): vscode.DocumentFilter[] {
	...
	function asPattern(
		value: Types.GlobPattern | undefined | null,
...
class OutputChannelResource extends Resource.Default implements OutputChannel {
	public static $resources: ResourceManager<OutputChannel> =
		new ResourceManager.Default();
...
class TextDocumentResourceManager extends ResourceManager.Default<TextDocumentResource> {
	private readonly document2Handle: WeakMap<
		vscode.TextDocument,
		ResourceHandle<TextDocumentResource>
	> = new WeakMap();
...
class TextDocumentResource extends Resource.Default implements TextDocument {
	public static readonly $resources: TextDocumentResourceManager =
		new TextDocumentResourceManager();

	public static textDocument(
	...
	public constructor(
		...
		handle?: ResourceHandle<TextDocumentResource>,
	) {
		if (handle !== undefined) {
			super(handle);
		} else {
...
class TextDocumentChangeEventResource
	extends Resource.Default
	implements Types.TextDocumentChangeEvent
{
	public static readonly $resources: ResourceManager<Types.TextDocumentChangeEvent> =
		new ResourceManager.Default();
...
export async function activate(
	...
	const service: api.all.Imports = {
		...
		workspace: {
			...
			textDocuments: () => {
				return vscode.workspace.textDocuments.map((document) =>
					TextDocumentResource.$resources.getOrCreate(document),
				);
			},
		},
		commands: {
		...
		languages: {
			matchSelector: (
				selector: Types.DocumentSelector,
				document: Types.TextDocument,
			) => {
				if (selector.isSingle()) {
					return vscode.languages.match(
						Converter.asDocumentFilter(selector.value),
						TextDocumentResource.textDocument(document),
					);
				} else if (selector.isMany()) {
					return vscode.languages.match(
						Converter.asDocumentFilters(selector.value),
						TextDocumentResource.textDocument(document),
					);
				} else {
					return 0;
