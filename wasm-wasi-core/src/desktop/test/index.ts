	...
	protected async handleMessage(message: WorkerMessage): Promise<void> {
		if (TestsDoneMessage.is(message)) {
			if (message.failures > 0) {
				this._testDone.reject(
					new Error(`${message.failures} tests failed.`)
				);
			} else {
...
export async function run(_testRoot: string): Promise<void> {
	...
	const files = (
		await glob("**/**main.test.js", { cwd: commonTestRoot })
	).map((f) => path.resolve(commonTestRoot, f));
...
async function doRunWorkerTests(
	...
	const message: TestSetupMessage = {
		method: "testSetup",
		shared,
		stats: workspaceContent.stats,
	};
