...
namespace FileSystem {
	...
	export function createFolder(
		...
		const path = memory.allocString(name);
		let errno = wasi.path_create_directory(
			parentFd,
			path.$ptr,
			path.byteLength
		);
	...
	export function createFileClose(
		memory: Memory,
		parentFd: fd,
		filename: string,
		content: Uint8Array | string
...
suite(`Simple test - ${memoryQualifier}`, () => {
	test("argv", () => {
		...
		const expectedBufferLength = expectedArgs.reduce<number>(
			(previous, value) => {
	...
	test("clock", () => {
		...
		assert.ok(
			now - delta < time.value && time.value < now + delta,
			`realtime [${now}, ${time.value}]`
		);
		...
		assert.ok(
			hrtime - delta < time.value && time.value < hrtime + delta,
			`monotonic [${hrtime}, ${time.value}]`
		);
	...
	test("env", () => {
		...
		let errno = wasi.environ_sizes_get(
			environCount.$ptr,
			environBufSize.$ptr
		);
...
suite(`Filesystem - ${memoryQualifier}`, () => {
	...
	test("fd_read - single iovec", () => {
		...
		assert.strictEqual(
			decoder.decode(memory.readBytes(buffer.$ptr, bytesRead.value)),
			content
		);
		FileSystem.close(fd);
	});
	...
	test("fd_read - multiple iovec", () => {
		...
		for (let i = 0; i < iovecs.size; i++) {
			...
			assert.strictEqual(
				decoder.decode(memory.readBytes(iovec.buf, toRead)),
				"1".repeat(toRead)
			);
			rest -= toRead;
		}
	...
	test("path_open - truncate file", () => {
		...
		const filename = `/tmp/${uuid.v4()}`;
		FileSystem.close(
			FileSystem.createFile(memory, rootFd, filename, "Hello World")
		);
		const fd = memory.allocUint32();
	...
	test("path_open - append mode", () => {
		...
		const filename = `/tmp/${uuid.v4()}`;
		FileSystem.close(
			FileSystem.createFile(memory, rootFd, filename, "Hello")
		);
		const fd = memory.allocUint32(0);
		const p = memory.allocString(filename);
		let errno = wasi.path_open(
		...
		const check = FileSystem.openFile(memory, rootFd, filename);
		assert.strictEqual(
			decoder.decode(FileSystem.read(memory, check)),
			"Hello World"
		);
	});
	...
	test("fd_advise - sequential", () => {
		const memory = createMemory();
		const filename = `/tmp/${uuid.v4()}`;
		const fd = FileSystem.createFile(
			memory,
			rootFd,
			filename,
			"Hello World"
	...
	test("fd_filestat_set_times", () => {
		...
		const content = "Hello World";
		const fd = FileSystem.createFile(memory, rootFd, filename, content);
		const errno = wasi.fd_filestat_set_times(
			fd,
			10n,
			10n,
	...
	test("fd_readdir - single read", () => {
		...
		while (index < end) {
			...
			const name = decoder.decode(
				memory.readBytes(index + Dirent.size, dirent.d_namlen)
			);
	...
	test("fd_readdir - multiple read", () => {
		...
		do {
			...
			while (
				...
				index - buffer.$ptr < bufUsed.value
			) {
				const dirent = memory.readStruct(index, Dirent);
				...
				if (spaceLeft >= dirent.d_namlen) {
					const name = decoder.decode(
						memory.readBytes(index, dirent.d_namlen)
					);
					assert.ok(fileNames.has(name), "Known file name");
					fileNames.delete(name);
	...
	test("path_filestat_get", () => {
		...
		const filestat = memory.allocStruct(Filestat);
		let errno = wasi.path_filestat_get(
			rootFd,
			Lookupflags.none,
			path.$ptr,
			path.byteLength,
	...
	test("path_readlink", () => {
		...
		const buffer = memory.alloc(1024);
		const bufUsed = memory.allocUint32();
		const errno = wasi.path_readlink(
			rootFd,
			path.$ptr,
			path.byteLength,
			buffer.$ptr,
