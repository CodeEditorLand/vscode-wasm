...
export async function createWorkspaceContent(): Promise<WorkspaceContent> {
	...
	const helloWorld = folder.with({
		path: path.join(read.path, "helloWorld.txt"),
	});
	...
	await vscode.workspace.fs.writeFile(
		folder.with({ path: path.join(read.path, "large.txt") }),
		encoder.encode("1".repeat(3000))
	);
...
export async function cleanupWorkspaceContent(
	workspaceContent: WorkspaceContent
): Promise<void> {
	await vscode.workspace.fs.delete(workspaceContent.tmp, { recursive: true });
	await vscode.workspace.fs.delete(workspaceContent.fixture, {
		recursive: true,
	});
}

export function createWasiService(
	workspaceContent: WorkspaceContent
): WasiService {
	const fileDescriptors: FileDescriptors = new FileDescriptors();
	const deviceDrivers = WasiKernel.deviceDrivers;
	const fileSystem = vscfs.create(
		WasiKernel.nextDeviceId(),
		workspaceContent.root
	);
	...
	const fileSystemService = DeviceWasiService.create(
		deviceDrivers,
		fileDescriptors,
		clock,
		undefined,
	...
	const environmentService = EnvironmentWasiService.create(
		fileDescriptors,
		"testApp",
		preOpenDirectories.entries(),
		options
	);
	const clockService = ClockWasiService.create(clock);
	const result: WasiService = Object.assign(
		{},
		NoSysWasiService,
		environmentService,
		clockService,
