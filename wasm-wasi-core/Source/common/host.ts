		...
		const wasi: WasiHost = {
			...
			path_filestat_get: (
				...
				try {
					return connection.call(
						path_filestat_get,
						[fd, flags, path_ptr, path_len, filestat_ptr],
						memory(),
			...
			poll_oneoff: (
				...
				try {
					return connection.call(
						poll_oneoff,
						[input, output, subscriptions, result_size_ptr],
						memory(),
						poll_oneoff.transfers(
							memoryView(),
							input,
							output,
							subscriptions,
