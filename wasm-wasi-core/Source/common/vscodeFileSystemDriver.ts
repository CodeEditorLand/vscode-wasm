...
const _DirectoryInheritingRights: rights =
...
const DirectoryOnlyBaseRights: rights =
	getDirectoryBaseRights() & ~getFileBaseRights();
const FileOnlyBaseRights: rights =
	getFileBaseRights() & ~getDirectoryBaseRights();
const StdInFileRights: rights =
	Rights.fd_read |
	Rights.fd_seek |
	Rights.fd_tell |
	Rights.fd_advise |
...
class FileSystem {
	...
	public getUri(node: DirectoryNode, fsPath?: string): Uri {
		...
		const finalPath =
			fsPath === undefined || fsPath === "."
				? this.getPath(node)
				: paths.join(this.getPath(node), fsPath);
		return this.vscfs.with({
			path: paths.join(this.vscfs.path, finalPath),
		});
	}
	...
	public deleteNode(node: Node, stat?: FileStat, content?: Uint8Array): void {
		...
		if (
			node.refs > 0 &&
			(stat === undefined ||
				(node.kind === NodeKind.File && content === undefined))
		) {
			throw new WasiError(Errno.inval);
		}
...
export function create(
	...
	function createFileDescriptor(
		parentDescriptor: DirectoryFileDescriptor,
		fd: fd,
		rights_base: rights,
		fdflags: fdflags,
		path: string
	...
	function createDirectoryDescriptor(
		parentDescriptor: DirectoryFileDescriptor,
		fd: fd,
		rights_base: rights,
		rights_inheriting: rights,
	...
	const $this: FileSystemDeviceDriver = {
		...
		createStdioFileDescriptor(
			...
			const fs_rights_base: rights =
				_fs_rights_base ?? fd === 0
					...
					: StdoutFileRights;
			const oflags: oflags =
				_oflags ?? fd === 0 ? Oflags.none : Oflags.creat | Oflags.trunc;
		...
		path_link(
			_oldFileDescriptor: FileDescriptor,
		...
		async path_open(
			...
			if (Oflags.creatOn(oflags) && !entryExists) {
				...
				if (dirname !== ".") {
					...
					if (
						...
						dirFiletype !== Filetype.directory
					) {
						throw new WasiError(Errno.noent);
					}
				}
			...
			if (
				...
				(createFile || Oflags.truncOn(oflags))
			) {
