...
class FileSystem {
	...
	private processDirectoryNode(
		dump: Dump.DirectoryNode,
		fs: DirectoryNode,
		inodeCounter: bigint,
	): void {
		for (const entry of Object.values(dump.children)) {
			if (entry.kind === "directory") {
				const child = DirectoryNode.create(
					fs,
					inodeCounter++,
					entry.name,
					BigInt(entry.ctime),
	...
	const $driver: ReadonlyFileSystemDeviceDriver = {
			...
			const result =
				target.kind === NodeKind.Directory
					? new DirectoryFileDescriptor(
							deviceId,
							fdProvider.next(),
							fileDescriptor.childDirectoryRights(fs_rights_base),
		...
		fd_bytesAvailable(fileDescriptor: FileDescriptor): Promise<filesize> {
			...
			return Promise.resolve(
				BigInts.max(
					0n,
					fileDescriptor.node.size - fileDescriptor.cursor,
				),
			);
		},
	};
