...
namespace VirtualDirectoryNode {
	export function create(
		id: inode,
		parent: VirtualDirectoryNode | undefined,
		name: string,
...
class VirtualRootFileSystem {
	...
	public addMountPoint(
		...
		for (let i = 0; i < segments.length; i++) {
			if (current.kind === NodeKind.MountPoint) {
				throw new Error(
					`Cannot create virtual folder over mount point: ${
						path.sep
					}${segments.slice(0, i + 1).join(path.sep)}`,
				);
			}
	...
	public findNode(
		...
		if (filePath === "/") {
			...
			return parentNode.kind === NodeKind.VirtualDirectory
		...
		for (let i = 0; i < segments.length; i++) {
			...
			if (child === undefined) {
				...
				return child.kind === NodeKind.VirtualDirectory
					? [child, undefined]
					: [child, "."];
			}
			current = child;
		...
		for (const [mountPoint, node] of this.mountPoints) {
			...
			if (
				...
				(uriStr.startsWith(rootStr) &&
					(rootStr.charAt(rootStr.length - 1) === "/" ||
...
export function create(
	...
	const $ctim: bigint = $atim;

	function assertDirectoryDescriptor(
		fileDescriptor: FileDescriptor,
	): asserts fileDescriptor is FileDescriptor & {
	...
	const $driver = {
		...
		createStdioFileDescriptor(): Promise<FileDescriptor> {
			throw new Error(
				`Virtual root FS can't provide stdio file descriptors`,
			);
		},
		...
		async path_filestat_get(
			...
			if (
				...
				return this.fd_filestat_get(
					rootFileDescriptors.getRoot($this)!,
					result,
				);
			}
