...
import type { i32, own, u32 } from "@vscode/wasm-component-model";
...
export namespace Window.$ {
	export const TestResource = new $wcm.ResourceType<Window.TestResource>(
		"test-resource",
	...
	export const createTestResource =
		new $wcm.FunctionType<Window.createTestResource>(
			...
			[],
			new $wcm.OwnType<Window.TestResource>(TestResource),
		);
}
...
export namespace test.$ {
	export namespace exports {
		export const run = new $wcm.FunctionType<test.Exports["run"]>(
			"run",
			[],
			undefined,
		);
...
export namespace test._ {
	...
	): Promise<test.Exports> | Promise<test.Exports.Promisified> {
