...
type _SyncCallHandler = (
	params: any,
	result?: MemoryRange,
) => HandlerResult<number | undefined>;
...
type _AsyncCallSignatures<
	AsyncCalls extends _AsyncCallType,
	TLI = TransferItems,
> = UnionToIntersection<
	{
		[call in AsyncCalls as call["method"]]: call["params"] extends object
			? (
					...
					call["result"] extends undefined | void
						? void
						: call["result"]
				>
			: (
...
type AsyncCallSignatures<AsyncCalls extends _AsyncCallType | undefined, TLI> = [
	AsyncCalls,
] extends [_AsyncCallType]
	? _AsyncCallSignatures<AsyncCalls, TLI>
	: undefined;

type _AsyncCallHandlerSignatures<AsyncCalls extends _AsyncCallType> =
	UnionToIntersection<
		{
			[call in AsyncCalls as call["method"]]: call["params"] extends object
				? (
...
type AsyncCallHandlerSignatures<AsyncCalls extends _AsyncCallType | undefined> =
	[AsyncCalls] extends [_AsyncCallType]
		? _AsyncCallHandlerSignatures<AsyncCalls>
		: undefined;

type _SyncCallSignatures<
	SyncCalls extends _SyncCallType,
	TLI = TransferItems,
> = UnionToIntersection<
	{
		[call in SyncCalls as call["method"]]: call["params"] extends object
			? call["result"] extends _SharedObjectResult
				? (
						method: call["method"],
						params: call["params"],
						resultDescriptor: _SharedObjectResult,
						timeout?: number | undefined,
						transferList?: ReadonlyArray<TLI>,
					) => InstanceType<call["result"]>
				: (
						method: call["method"],
						params: call["params"],
						timeout?: number | undefined,
...
type SyncCallSignatures<
	SyncCalls extends _SyncCallType | undefined,
	TLI = TransferItems,
> = [SyncCalls] extends [_SyncCallType]
	? _SyncCallSignatures<SyncCalls, TLI>
	: undefined;

type _SyncCallHandlerSignatures<SyncCalls extends _SyncCallType> =
	UnionToIntersection<
		{
			[call in SyncCalls as call["method"]]: call["params"] extends object
				? call["result"] extends _SharedObjectResult
				...
				: call["result"] extends _SharedObjectResult
					...
					: (
							method: call["method"],
							handler: () => HandlerResult<number | void>,
						) => void;
		}[keyof MethodKeys<SyncCalls>]
	>;

type SyncCallHandlerSignatures<SyncCalls extends _SyncCallType | undefined> = [
	SyncCalls,
] extends [_SyncCallType]
	? _SyncCallHandlerSignatures<SyncCalls>
	: undefined;

type _NotifySignatures<
	Notifications extends _NotifyType,
	TLI = TransferItems,
> = UnionToIntersection<
	{
...
type NotifySignatures<
	Notifications extends _NotifyType | undefined,
	TLI = TransferItems,
> = [Notifications] extends [_NotifyType]
	? _NotifySignatures<Notifications, TLI>
	: undefined;

type _NotifyHandlerSignatures<Notifications extends _NotifyType> =
	UnionToIntersection<
		{
			[N in Notifications as N["method"]]: N["params"] extends object
				? (
...
type NotifyHandlerSignatures<Notifications extends _NotifyType | undefined> = [
	Notifications,
] extends [_NotifyType]
	? _NotifyHandlerSignatures<Notifications>
	: undefined;
...
export abstract class BaseConnection<
	AsyncCalls extends _AsyncCallType | undefined,
	SyncCalls extends _SyncCallType | undefined,
	Notifications extends _NotifyType | undefined,
	AsyncCallHandlers extends _AsyncCallType | undefined = undefined,
	SyncCallHandlers extends _SyncCallType | undefined = undefined,
	NotifyHandlers extends _NotifyType | undefined = undefined,
	TLI = TransferItems,
> {
	...
	public readonly callAsync: AsyncCallSignatures<AsyncCalls, TLI> = this
		._callAsync as AsyncCallSignatures<AsyncCalls, TLI>;
	...
	public readonly onAsyncCall: AsyncCallHandlerSignatures<AsyncCallHandlers> =
		this._onAsyncCall as AsyncCallHandlerSignatures<AsyncCallHandlers>;
	...
	public initializeSyncCall(memory: SharedMemory): void {
		...
		this.syncLocation = {
			...
			size: this.syncMemoryRange.size,
		};
	}
	...
	public readonly callSync: SyncCallSignatures<SyncCalls, TLI> = this
		._callSync as SyncCallSignatures<SyncCalls, TLI>;

	private _callSync(
		method: string,
		params?: any,
		result?: _SharedObjectResult | undefined,
		timeout?: number | undefined,
		transferList?: ReadonlyArray<TLI>,
	): any {
		if (
			this.syncMemoryRange === undefined ||
			this.syncLocation === undefined ||
			this.memory === undefined
		) {
			throw new Error(
		...
		const handleResult = (): SharedObject | undefined => {
			...
			if (error === 0) {
				...
				throw new SyncCallError(
					`Sync call ${method} failed with error code ${error}`,
					method,
					error,
				);
			}
		...
		switch (wait) {
			...
			case "not-equal":
				...
				if (current === 1) {
					...
					throw new Error(
						`Sync call ${method} failed with unexpected value ${current}`,
					);
				}
		}
	}

	public readonly onSyncCall: SyncCallHandlerSignatures<SyncCallHandlers> =
		this._onSyncCall as SyncCallHandlerSignatures<SyncCallHandlers>;
	...
	public readonly notify: NotifySignatures<Notifications, TLI> = this
		._notify as NotifySignatures<Notifications, TLI>;
	...
	public readonly onNotify: NotifyHandlerSignatures<NotifyHandlers> = this
		._onNotify as NotifyHandlerSignatures<NotifyHandlers>;
	...
	protected abstract postMessage(
		message: _Message | _AsyncResponse,
		transferList?: ReadonlyArray<TLI>,
	): void;
				...
				try {
					errorCode =
						(await handler(
							message.params,
							message.result
								? this.memory.range.fromLocation(message.result)
								: undefined,
						)) ?? 0;
				} catch (error: any) {
...
export type AnyConnection = BaseConnection<
	AnyConnection.AsyncCall,
